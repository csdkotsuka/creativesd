...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "leaflet_output/inteve_link")
FILES_DIR = os.path.join(PROJECT_ROOT, "files")
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(FILES_DIR, exist_ok=True)

CHROME_BIN = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
HTML_PATH = os.path.join(PROJECT_ROOT, "inteve_link_leaflet_4p.html")

with open(HTML_PATH, "r", encoding="utf-8") as f:
    full_html = f.read()

# Replace relative image paths with absolute file:// URLs so headless Chrome loads them perfectly
...
abs_html_file = "/tmp/inteve_link_leaflet_4p_abs.html"
with open(abs_html_file, "w", encoding="utf-8") as f:
    f.write(full_html_abs)

print("1. Generating High-Res A4 4-Page PDF...")
pdf_path = os.path.join(OUTPUT_DIR, "INTEVE_LINK_Leaflet_A4.pdf")
cmd_pdf = [
    CHROME_BIN,
    "--headless",
    "--disable-gpu",
...
# Copy to files/ directory for public download
final_pdf_path = os.path.join(FILES_DIR, "INTEVE_LINK_Leaflet_A4.pdf")
shutil.copyfile(pdf_path, final_pdf_path)
os.chmod(final_pdf_path, 0o644)
print(f"Copied to files: {final_pdf_path} ({os.path.getsize(final_pdf_path)} bytes)")
...
pages = [
    ("page_1_cover.jpg", 1),
    ("page_2_daily_and_safety.jpg", 2),
    ("page_3_clinical_learning.jpg", 3),
    ("page_4_impact_and_specs.jpg", 4),
]
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    <div style="width: 1200px; height: 1697px; box-sizing: border-box; overflow: hidden;">
        ...
        temp_html_path = f"/tmp/inteve_link_page_{page_num}_abs.html"
        with open(temp_html_path, "w", encoding="utf-8") as tf:
            tf.write(page_html_content)

        temp_png = f"/tmp/inteve_link_page_{page_num}.png"
        cmd_shot = [
            CHROME_BIN,
            "--headless",
            "--disable-gpu",
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    ...
    temp_s1_html = "/tmp/inteve_link_spread_1_abs.html"
    with open(temp_s1_html, "w", encoding="utf-8") as tf:
        tf.write(spread1_html)
    temp_s1_png = "/tmp/inteve_link_spread_1.png"
    subprocess.run([
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    ...
    temp_s2_html = "/tmp/inteve_link_spread_2_abs.html"
    with open(temp_s2_html, "w", encoding="utf-8") as tf:
        tf.write(spread2_html)
    temp_s2_png = "/tmp/inteve_link_spread_2.png"
    subprocess.run([
        CHROME_BIN, "--headless", "--disable-gpu", "--virtual-time-budget=5000",
    ...
    # A3 Spread PDF
    spread_pdf_path = os.path.join(OUTPUT_DIR, "INTEVE_LINK_Leaflet_A3_Spread.pdf")
...
<body class="bg-white text-slate-800 p-0 m-0 font-sans">
    <div class="spread-sheet">
        <div class="spread-half">{sections[3].replace('class="leaflet-page', 'class="leaflet-page w-full h-full shadow-none rounded-none')}</div>
        <div class="spread-half">{sections[0].replace('class="leaflet-page', 'class="leaflet-page w-full h-full shadow-none rounded-none')}</div>
    </div>
    <div class="spread-sheet">
        <div class="spread-half">{sections[1].replace('class="leaflet-page', 'class="leaflet-page w-full h-full shadow-none rounded-none')}</div>
        <div class="spread-half">{sections[2].replace('class="leaflet-page', 'class="leaflet-page w-full h-full shadow-none rounded-none')}</div>
    </div>
</body>
</html>"""
    temp_spread_pdf_html = "/tmp/inteve_link_spread_pdf_abs.html"
    ...
    print(f"Generated: {spread_pdf_path} ({os.path.getsize(spread_pdf_path)} bytes)")

print("\n--- All INTEVE LINK Leaflet Assets Successfully Generated! ---")
