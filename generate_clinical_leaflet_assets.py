...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "leaflet_output/clinical")
FILES_DIR = os.path.join(PROJECT_ROOT, "files")
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(FILES_DIR, exist_ok=True)

CHROME_BIN = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
HTML_PATH = os.path.join(PROJECT_ROOT, "clinical_leaflet_4p.html")
...
final_pdf_path = os.path.join(FILES_DIR, "CLINICAL_Leaflet_A4.pdf")
...
pages = [
    ("page_1_cover.jpg", 1),
    ("page_2_placement_matching.jpg", 2),
    ("page_3_faculty_and_docs.jpg", 3),
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    <div style="width: 1200px; height: 1697px; box-sizing: border-box; overflow: hidden;">
        ...
        temp_html_path = f"/tmp/clinical_page_{page_num}_abs.html"
        with open(temp_html_path, "w", encoding="utf-8") as tf:
            tf.write(page_html_content)

        temp_png = f"/tmp/clinical_page_{page_num}.png"
        cmd_shot = [
            CHROME_BIN,
            "--headless",
            "--disable-gpu",
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    ...
    temp_s1_html = "/tmp/clinical_spread_1_abs.html"
    with open(temp_s1_html, "w", encoding="utf-8") as tf:
        tf.write(spread1_html)
    temp_s1_png = "/tmp/clinical_spread_1.png"
    subprocess.run([
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    ...
    temp_s2_html = "/tmp/clinical_spread_2_abs.html"
    with open(temp_s2_html, "w", encoding="utf-8") as tf:
        tf.write(spread2_html)
    temp_s2_png = "/tmp/clinical_spread_2.png"
    subprocess.run([
        CHROME_BIN, "--headless", "--disable-gpu", "--virtual-time-budget=5000",
    ...
    # A3 Spread PDF
    spread_pdf_path = os.path.join(OUTPUT_DIR, "CLINICAL_Leaflet_A3_Spread.pdf")
...
<body class="bg-white text-slate-800 p-0 m-0 font-sans">
    ...
    temp_spread_pdf_html = "/tmp/clinical_spread_pdf_abs.html"
...
print("\n--- All Clinical Leaflet Assets Successfully Generated! ---")
