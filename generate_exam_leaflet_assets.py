...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "leaflet_output/exam")
os.makedirs(OUTPUT_DIR, exist_ok=True)

CHROME_BIN = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
HTML_PATH = os.path.join(PROJECT_ROOT, "exam_leaflet_4p.html")
...
pages = [
    ("page_1_cover.jpg", 1),
    ("page_2_student_flow.jpg", 2),
    ("page_3_faculty_features.jpg", 3),
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    <div style="width: 1200px; height: 1697px; box-sizing: border-box; overflow: hidden;">
        ...
        temp_html_path = f"/tmp/exam_page_{page_num}_abs.html"
        with open(temp_html_path, "w", encoding="utf-8") as tf:
            tf.write(page_html_content)

        temp_png = f"/tmp/exam_page_{page_num}.png"
        cmd_shot = [
            CHROME_BIN,
            "--headless",
            "--disable-gpu",
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    ...
    temp_s1_html = "/tmp/exam_spread_1_abs.html"
    with open(temp_s1_html, "w", encoding="utf-8") as tf:
        tf.write(spread1_html)
    temp_s1_png = "/tmp/exam_spread_1.png"
    subprocess.run([
...
<body class="bg-white text-slate-800 p-0 m-0 overflow-hidden flex justify-center items-center font-sans">
    ...
    temp_s2_html = "/tmp/exam_spread_2_abs.html"
    with open(temp_s2_html, "w", encoding="utf-8") as tf:
        tf.write(spread2_html)
    temp_s2_png = "/tmp/exam_spread_2.png"
    subprocess.run([
        CHROME_BIN, "--headless", "--disable-gpu", "--virtual-time-budget=5000",
    ...
    # A3 Spread PDF
    spread_pdf_path = os.path.join(OUTPUT_DIR, "EXAM_Leaflet_A3_Spread.pdf")
...
<body class="bg-white text-slate-800 p-0 m-0">
    ...
    temp_spread_pdf_html = "/tmp/exam_spread_pdf_abs.html"
